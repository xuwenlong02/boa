    ...
    fn parse(
        ...
        loop {
            ...
            params.push(if cursor.next_if(Punctuator::Spread).is_some() {
                ...
                FormalParameter::new(self.allow_yield, self.allow_await).parse(cursor, interner)?
            });
            ...
            if rest_param {
                let prev_tok = cursor.peek_prev().expect("current token disappeared");
                return Err(ParseError::unexpected(
                    prev_tok.display(interner).to_string(),
                    prev_tok.pos,
